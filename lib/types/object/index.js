    ...
    _base(value, state, options) {
        ...
        if (this._inner.children) {
            ...
            for (let i = 0; i < this._inner.children.length; ++i) {
                ...
                const key = child.key;
                const replaceKey = child.schema._rename || key;
                ...
                if (result.errors) {
                    ...
                    if (child.schema._flags.strip || (result.value === undefined && result.value !== item)) {
                        stripProps.push(replaceKey);
                        target[replaceKey] = result.finalValue;
                        (replaceKey !== key) && delete target[key];
                    }
                    else if (result.value !== undefined) {
                        target[replaceKey] = result.value;
                        (replaceKey !== key) && delete target[key];
                    }
                }
            }
